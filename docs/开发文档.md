...
>
> **v1.6 更新**：单容器全栈部署——容器内由 supervisord 编排 **PostgreSQL + Redis + Celery Worker + Nginx + FastAPI**（仍仅一个容器对外），数据卷 `/data` 持久化；async 审核优先 Celery 投递、Redis 不可用时自动降级进程内后台任务。
>
> v1.5 更新：部署精简为单容器（SQLite 卷持久化、内存缓存降级）；`AI_MODE=mock` 本地演示模式；新增 `AI_VISION_MODEL` 图片审核视觉模型配置。
>
...
├── deploy/                    # 部署文件
│   ├── docker-compose.yml     # 编排：单容器全栈（PG + Redis + Celery + Nginx + FastAPI，v1.6）
│   ├── entrypoint.sh          # 启动脚本（首次初始化 PG → exec supervisord）
│   ├── supervisord.conf       # 容器内进程编排（postgres/redis/nginx/uvicorn/celery）
│   ├── pg_init.sh             # PostgreSQL 首次初始化（initdb + 建库 forum）
│   ├── redis.conf             # 容器内 Redis 配置（127.0.0.1 + AOF）
│   ├── nginx/
...
│  ├─ postgres :5432 PostgreSQL（/data/postgres）       │
│  ├─ redis    :6379 Redis 缓存/队列（/data/redis）     │
│  └─ celery worker   AI 审核异步任务（连容器内 Redis）  │
│ 数据卷 /data：postgres 数据、redis AOF、uploads        │
└──────────────────────────────────────────────────────┘
```

- PostgreSQL / Redis 仅监听容器内 `127.0.0.1`，compose 不映射端口，外部不可直接访问；
- AI 审核 `async` 模式优先投递 Celery（容器内 worker），Redis 不可用时自动降级进程内 asyncio 后台任务；
- 安全说明：supervisord 以 root 启动以拉起 postgres（内部 setuid postgres）；nginx/uvicorn/celery 均以非 root 用户运行。

**大规模扩展（可选）**：多实例时可将 PostgreSQL / Redis 外置（环境变量指向外部服务）：

```
Nginx / LB
  └── FastAPI 实例 × N ──> PostgreSQL（主从）
                       ──> Redis（主从 + 哨兵）
Celery Worker × N（独立容器，任务代码已保留兼容）
```

### 10.2 Docker Compose（单容器全栈，默认部署）

> 仓库 `deploy/docker-compose.yml` 即实际编排文件；合并镜像（根目录 `Dockerfile` 多阶段：前端构建 →
> Python 运行层）**仅一个容器**，容器内 supervisord 编排 PostgreSQL / Redis / Nginx / FastAPI / Celery Worker。
...
services:
  forum:
    ...
    environment:
      # 容器内服务（supervisord 编排，仅监听 127.0.0.1）
      DATABASE_URL: postgresql+asyncpg://forum:forum@127.0.0.1:5432/forum
      REDIS_URL: redis://127.0.0.1:6379/0
      UPLOAD_DIR: /data/uploads
      SECRET_KEY: ${SECRET_KEY:-dev-only-secret-key-change-me}
      CORS_ORIGINS: ${CORS_ORIGINS:-http://localhost}
      # AI 审核（两级审核：AI 初审 + 人工复审）
      ...
      AI_AUDIT_MODE: ${AI_AUDIT_MODE:-async}             # sync 请求内 / async Celery 投递 / off
    volumes:
      - forumdata:/data                                  # postgres 数据 / redis AOF / uploads
    ports:
      - "80:8080"                                        # 前端 + /api（PG/Redis 不对外暴露）
...
> 启动：`docker compose -f deploy/docker-compose.yml up -d --build`（首次自动 initdb 建库）
> 升级 / 回滚：`docker compose pull && docker compose up -d`；镜像发布见 `.github/workflows/docker-publish.yml`（推送 `v*.*.*` 标签自动构建发布 GHCR）。
>
> **扩展部署**：多实例时将 PostgreSQL / Redis 外置（`DATABASE_URL` / `REDIS_URL` 指向外部服务），
> 并在独立容器运行 `celery -A app.tasks worker`（任务代码已保留兼容，见 10.1「大规模扩展」）。

### 10.3 环境变量清单（后端）

| 变量 | 示例 | 说明 |
| --- | --- | --- |
| `DATABASE_URL` | `postgresql+asyncpg://forum:forum@127.0.0.1:5432/forum` | 数据库连接串（单容器内联 PostgreSQL；本地开发可用 `sqlite+aiosqlite:///./forum.db`） |
| `REDIS_URL` | `redis://127.0.0.1:6379/0` | Redis 连接串（不可用时自动降级内存存储；Celery 投递降级进程内任务） |
...
```bash
# 1. 单容器全栈部署（推荐，仅一个容器：PG + Redis + Celery + 前端 + 后端）
docker compose -f deploy/docker-compose.yml up -d --build
# 访问 http://localhost （前端 + /api）；首次启动自动初始化 PostgreSQL

# 2. 本地开发（前后端分离，SQLite + 内存缓存降级）
# 后端
cd backend
