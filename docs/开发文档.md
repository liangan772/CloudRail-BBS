...
>
> **v1.2 更新**：新增国内主流论坛社区功能（轮播图、话题广场、投票帖、草稿箱、举报、拉黑、浏览历史、推荐流、公告中心）与移动端底部导航栏规范；并基于全文档审计修复一致性问题。
...
│   │   │   └── deps.py        # 依赖注入（当前用户等）
│   │   ├── models/            # SQLAlchemy 模型（含 gamification / oauth / devices）
│   │   ├── schemas/           # Pydantic 模型
│   │   ├── services/          # 业务逻辑层（含积分、签到、推送服务）
│   │   ├── tasks/             # Celery 任务（含 push / sms / rank）
│   │   ├── utils/             # 工具（敏感词、分页等）
│   │   └── main.py            # 应用入口
│   ├── alembic/               # 数据库迁移
│   ├── tests/                 # pytest 测试
...
| M6 管理后台 | 用户/内容/分类/敏感词管理、激励配置、轮播图、举报处理、话题运营、运营看板 | 2 周 |
