...
>
> **v1.3 更新**：新增 AI 自动审核（LLM 内容安全审核：同步接口 + 异步任务 + 审核记录表）；修复 Docker 构建（Dockerfile 迁移至各模块标准位置）。
...
- 目标：发帖 / 评论前进行内容安全审核，降低违规内容对社区的冲击，减轻人工审核压力。
- 审核模式（`AI_AUDIT_MODE` 配置）：`sync` 同步（先审后发）/ `async` 异步（先发后审，违规自动下架）/ `off` 关闭。
- 模型接入：OpenAI 兼容协议（DeepSeek / 通义千问 / 智谱等），通过 `AI_BASE_URL` + `AI_API_KEY` + `AI_MODEL` 配置，无需改代码即可切换供应商。
- 审核结论：`pass`（通过）/ `review`（转人工）/ `reject`（拦截），附违规分（0-100）、命中类别与理由。
- 记录可追溯：每次审核写入 `audit_records` 表，管理后台可查询与人工复核。
...
- `audit_records`（AI 审核记录）：id, target_type(`post`/`comment`/`user`), target_id, content, result(`pass`/`review`/`reject`), score(Float 0-100), categories(JSON 数组字符串), reason, model, created_at；索引 `(target_type, target_id)`、`(result, created_at)`；每次 LLM 审核落一条，支持追溯与人工复核。
...
> 集成说明：发帖 / 评论接口在 `AI_AUDIT_MODE=sync` 时先调用审核再发布；`async` 时发布后投递 `audit_content` 任务，reject 自动下架并通知管理员（见 9.16）。
...
├── deploy/                    # 部署文件
│   ├── docker-compose.yml     # 编排：PostgreSQL + Redis + 合并镜像 + Worker + Beat
...
- **接入方式**：OpenAI 兼容 `/chat/completions` 协议，`temperature=0` + `response_format=json_object` 保证输出稳定；供应商可切换（DeepSeek / 通义千问 / 智谱）。
- **Prompt 设计**：系统提示词固定输出 JSON（`result` / `score` / `categories` / `reason`），服务端对模型输出做归一化兜底（非法 result 降级为 `review`，score 钳制 0-100）。
- **调用链**：`POST /audit/text`（同步，登录用户）走 `AIAuditService.audit_text`（异步 httpx）；Celery 任务 `audit_content` 走 `audit_text_sync`（同步 httpx），两者共用 Prompt 与解析逻辑。
- **发布集成**：`AI_AUDIT_MODE=sync` 时发帖/评论先审核后发布（`reject` 直接拦截）；`async` 时发布后投递 `audit_content.delay()`，`reject` 结果自动将内容置为待审核并通知管理员。
- **记录与复核**：每次审核写入 `audit_records` 表（含目标、内容、结论、分数、类别、模型），管理后台 `GET /admin/audits` 查询并人工复核。
- **熔断与降级**：AI 服务超时/异常时同步模式放行（先发后审降级为人工），避免审核单点阻塞发帖链路；调用失败不阻塞核心业务。
...
客户端
  │ HTTPS
Nginx（静态前端 + 反向代理）
  ├── /api/*  → FastAPI（2 个实例，负载均衡）
  └── /static → 前端构建产物
FastAPI 实例 × N  ──> PostgreSQL（主从）
                 ──> Redis（主从 + 哨兵）
Celery Worker × N + Beat
```

### 10.2 Docker Compose（开发环境）

> 镜像策略：前后端打包为**单一镜像**（根目录 `Dockerfile`，多阶段构建：前端构建 → Python + Nginx 运行层）；
> 容器内 Nginx 托管前端静态资源并反代 `/api` 到本机 FastAPI（127.0.0.1:8000）。
> worker / beat 复用同一镜像，仅以 `command` 覆盖默认启动命令。
...
services:
  ...
  # 合并镜像：Nginx 前端（:80）+ FastAPI 后端（容器内 :8000）
  backend:
    build:
      context: .
      dockerfile: Dockerfile
    environment:
      DATABASE_URL: postgresql+asyncpg://forum:${PG_PASSWORD}@postgres:5432/forum
  ...
  worker:
    build:
      context: .
      dockerfile: Dockerfile
    command: sh -c "celery -A app.tasks worker --loglevel=info"

  beat:
    build:
      context: .
      dockerfile: Dockerfile
    command: sh -c "celery -A app.tasks beat --loglevel=info"
...
> 实际部署使用仓库中的 `deploy/docker-compose.yml`（context 为仓库根目录）。

### 10.3 环境变量清单（后端）

| 变量 | 示例 | 说明 |
| --- | --- | --- |
| `DATABASE_URL` | `postgresql+asyncpg://forum:pass@localhost:5432/forum` | PostgreSQL 连接串 |
...
| `AI_AUDIT_MODE` | `async` | 审核模式：sync 先审后发 / async 先发后审 / off 关闭 |
...
> 生产环境务必启用 HTTPS（Let's Encrypt / 云厂商证书），并配置 `client_max_body_size`（默认 1m，上传图片需调大，如 10m）。
